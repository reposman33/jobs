...
test.describe('Add Sollicitatie E2E Tests', () => {
  ...
  test('should successfully add a new sollicitatie', async ({ page }) => {
    ...
    // Aanvraag invullen
    await page.fill(
      'textarea[formControlName="aanvraag"]',
      'Dit is een test aanvraag voor een Senior Developer positie.'
    );
    ...
    // Motivatie invullen
    await page.fill(
      'textarea[formControlName="motivatie"]',
      'Ik ben zeer gemotiveerd om te werken bij TechCorp en wil graag mijn vaardigheden inzetten.'
    );

    // Updates invullen (optioneel)
    await page.fill(
      'textarea[formControlName="updates"]',
      'Eerste sollicitatie ronde afgerond'
    );
  ...
  test('should cancel form and empty form fields', async ({ page }) => {
    ...
    const motivatieInput = page.locator(
      'textarea[formControlName="motivatie"]'
    );
